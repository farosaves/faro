<script lang="ts">
	...
	let { supabase } = data;
	let session: { user: { id: any; email: any } };
	async function getSnippets() {
		const { data } = await supabase.from('snippets').select().eq('user_id', session.user.id);
		return data ?? [];
	}
	let snippets: SnippetData[] = [];

	const handleInserts = (payload: { new: SnippetData }) => {
		snippets.push(payload.new);
	};
