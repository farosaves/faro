...
function set(key, value, stringify2 = JSON.stringify) {
...
const states = get(STATES_KEY, parse) ?? {};
...
function create_client(app, target) {
  ...
  function persist_state() {
    ...
    set(STATES_KEY, states, stringify);
  }
  ...
  async function load_node({ loader, parent, url, params, route, server_data_node }) {
    ...
    return {
      ...
      slash: base && (url.pathname === base || url.pathname === base + "/") ? "always" : node.universal?.trailingSlash ?? server_data_node?.slash
    };
  }
  ...
  async function navigate({
    ...
    await tick();
    const scroll2 = popped ? popped.scroll : noscroll ? scroll_state() : null;
    if (autoscroll) {
      ...
      addEventListener("popstate", async (event) => {
        if (event.state?.[HISTORY_INDEX]) {
          ...
          if (history_index === current_history_index)
            return;
          const scroll2 = scroll_positions[history_index];
