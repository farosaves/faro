...
test("input8", async () => {
  ...
  expect(
    makeQCH(htmlstr2body)(document(r), r.uuid, r.selectedText).quote,
  ).not.toBe("Learn Flexbox with 30 Code Tidbits.")
})
test("input9", async () => {
...
test("context6", async () => {
  const r = await l("/input6.json")
  expect(
    makeQCH(htmlstr2body)(document(r), r.uuid, r.selectedText).context,
  ).toBe(
