<script lang="ts">
  ...
  import { matchField, required, pattern, email } from "svelte-forms/validators"
  const opt = { checkOnInit: true }
  const emailFld = field("email", "", [required(), email()], opt)
  ...
  const optEr = (err: string, msg: string) => ($myForm.dirty && $myForm.hasError(err) ? msg : "")
    ...
    <div class="card shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
      <form class="card-body" on:submit={submit}>
        <div class="form-control">
          ...
          <input id="email" type="email" class="input input-bordered" bind:value={$emailFld.value} required />
        </div>
        <div class="form-control">
        ...
        <!-- <div class="form-control">
          ...
          <input type="password" class="input input-bordered" required bind:value={$passwordConf.value} /> -->
        <!-- <label class="label">
              <a href="#" class="label-text-alt link link-hover"
                >Forgot password?</a>
            </label> -->
        <div class="form-control mt-6">
          <button disabled={!$myForm.valid} class="btn btn-primary">Submit</button>
        </div>
      </form>
    </div>
  </div>
