...
const load = async (s: string): Promise<HTMLElement> =>
  pipe(
    ...
    (x) => new JSDOM(x.toString()).window.document.body,
  )
...
test("subIdxs", async () => {
  expect(subIdxs(ser, 445, 489)).toBe(
    "type:textContent|445$489$1$_bce066f4-f72c-407f-a89e-6ab478629cfb$",
  )
})

test("extractQuote", async () => {
  expect(extractPrePost(ser + "henlo ev$eryone!")).toStrictEqual([
    "henlo ev",
    "eryone!",
  ])
